...
//		String sql="update applicants set sno=?,name=?,contact=?,email=?,experience=?,type=? where sno="+sno;
		try{
			Class.forName("com.mysql.jdbc.Driver");
			Connection conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/project","root","Ajay@123");
			PreparedStatement ps = conn.prepareStatement("update applicants set sno=?,empid=?,domain=?,requirment=?,date=?,priority=?,exp=?,typeof=? where sno="+sno);
